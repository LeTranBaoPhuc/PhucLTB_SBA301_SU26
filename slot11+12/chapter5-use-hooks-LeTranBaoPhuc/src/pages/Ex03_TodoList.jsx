    ...
    <Card className="mx-auto" style={{ maxWidth: 500 }}>
      ...
      <Card.Body>
        ...
        <ListGroup data-testid="todo-list" className="mb-3">
          {state.map(todo => (
            <ListGroup.Item key={todo.id} data-testid={`todo-item-${todo.id}`}
              style={{ textDecoration: todo.done ? 'line-through' : 'none' }}>
              <span>{todo.text}</span>
              <ButtonGroup className="float-end">
                <Button variant="success" size="sm" data-testid={`btn-toggle-${todo.id}`}
                  onClick={() => dispatch({ type: 'TOGGLE_TODO', payload: todo.id })}>
                  {todo.done ? 'Bỏ xong' : 'Xong'}
                </Button>
                <Button variant="danger" size="sm" data-testid={`btn-delete-${todo.id}`}
